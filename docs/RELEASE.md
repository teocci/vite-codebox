# Release index

Terse internal index of cut releases, newest first. The detailed public notes live in
`CHANGELOG.md`.

| Release | Date | Phases | Theme |
|---------|------|--------|-------|
| v0.9.0 | 2026-07-28 | P-15, P-16, P-17, P-18, P-19 | agent-directed presentation: viewer ops from protocol to CLI |
| v0.8.0 | 2026-07-27 | P-13, P-14 | the mirrors and the docs stop lying — a drift test for the s |
| v0.7.0 | 2026-07-27 | P-7, P-8, P-9, P-10, P-11, P-12 | builds land at true 1:1 — a scale gate that measures a plan  |
| v0.6.0 | 2026-07-26 | P-6 | Split App along its two domains: a type per concern over a s |
| v0.5.0 | 2026-07-26 | P-5 | codeblox-builder agent skill; per-verb flag validation and a |
| v0.4.0 | 2026-07-25 | 4 | Schema-driven build verbs: info, exec, ergonomic forms |
| v0.3.0 | 2026-07-25 | 3 | codeblox Go CLI: credentials, config, authenticated transport |
| v0.2.0 | 2026-07-25 | 1, 2 | Baseline: block engine, viewer, and authoritative WS server |
