...
- The running ws server was **stale**: a fresh contract fetch published only 8 ops, without P-8's `ellipsoid` and `tube`, though both are committed in `packages/shared/protocol.js`. Restarting the server is all it needs, but it means I-7 has not been live-verified against a server built from its own code.
