...
| [F-4](fixes/F-4.md) | At 1:1 scale the HUD's `extent` row renders 41 monospace characters — both unit systems in one value, and a block is 2 cm so the block triple is always 50× the metre triple. `.hud` sets `min-width` but no `max-width`, so the panel grows across the viewport. | (tbd) | (tbd) |  |
